...
<task type="auto">
  ...
  <action>
    ...
    ERROR RECOVERY: If a generation fails for any image, retry that specific image. Do not restart the full sequence. Already-generated images are saved to disk and do not need regeneration. Each image is independent once the style fingerprint is captured from the first successful generation.
