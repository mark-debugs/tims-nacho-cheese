...
NOTE: Feature cards on the homepage (West Coast Vibes, Bold Flavors, Skater Approved) use conceptual emojis (wave, fire, skateboard) that represent abstract ideas, not stock product imagery. These are intentionally out of scope for this phase. The phase goal "Replace stock iconography" targets product/merch imagery placeholders, not concept icons.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    **Do NOT change:**
    ...
    - Feature cards section (those use conceptual emojis for abstract ideas like wave/fire/skateboard, not product imagery -- intentionally out of scope)
    - Social proof stats section
    - CTA section
    - Any inview or animation logic
  </action>
  <verify>
    1. Run `bun run build` to verify build succeeds with enhanced:img processing
    2. Verify the import exists: grep for `import.*megaSpice.*from.*images/flavors` in src/routes/+page.svelte
    3. Verify enhanced:img is used: grep for `enhanced:img` in src/routes/+page.svelte
    4. Run dev server and check homepage loads with product image in hero and featured sections
    5. Check DevTools Network tab to confirm WebP/AVIF optimized formats are being served for the hero image
  </verify>
  <done>Homepage hero shows product image with spring animation, featured flavor section shows MEGA SPICE product image, build succeeds, optimized formats confirmed</done>
</task>
...
- Feature card concept emojis (wave, fire, skateboard) remain unchanged -- they are abstract concepts, not stock product imagery
</success_criteria>
