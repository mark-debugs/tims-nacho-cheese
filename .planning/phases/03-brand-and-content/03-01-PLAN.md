...
For each animated section, use a pattern like:
```svelte
<script>
...
<div use:inview on:inview_enter={onSectionInView}
     style="transform: translateY({$sectionY}px); opacity: {$sectionOpacity}">
  <!-- content -->
</div>
```

Note: The inview action dispatches `inview_enter` and `inview_exit` CustomEvents (check the actual event names in src/lib/actions/inview.ts before using them).
...
- All hover effects work (cheeseStretch on buttons/cards)
  ...
  <done>
Homepage displays animated hero with bouncy orange spring animations (HOME-01). About page tells Tim's story with scroll-triggered section animations (BRAND-01). All animations respect prefers-reduced-motion. Build passes.
  </done>
</task>
...
4. Homepage sections animate on scroll (feature cards, featured flavor, stats, CTA)
5. About page sections animate on scroll (story, philosophy, stats, essentials, values, CTA)
6. Hover effects work on interactive elements (cheeseStretch bounce)
7. With prefers-reduced-motion enabled in OS, all animations display instantly without motion
8. MDSveX configured in svelte.config.js and build passes with .md extension support
</verification>
